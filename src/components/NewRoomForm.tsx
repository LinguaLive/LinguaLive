...
const NewRoomForm = ({onClose}:NewRoomFormProps) => {
  ...
  for (const language of languages) {
    languagesList.push(<option value={language}>{language}</option>)
  };
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    const inputLanguage = form[2] as HTMLInputElement;
    if (inputName && inputName.value !== '' && inputLanguage) {
    ...
    } else if (inputName.value === '' && inputLanguage.value === '') {
      setEmptyName(true);
